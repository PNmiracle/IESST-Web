...
function emptyForm() {
  return { id: null, type: "SCI", title: "", field: "", indexType: "", cycle: "", description: "", documentName: "", documentUrl: "", published: true };
}
  ...
  <section class="admin-page">
    ...
    <div class="admin-layout">
      <form class="card editor" @submit.prevent="save">
        ...
        <label>类型<select v-model="form.type"><option>SCI</option><option>EI</option></select></label>
        <label>期刊名称<input v-model="form.title" required /></label>
        <label>学科方向<input v-model="form.field" required /></label>
        <label>检索类型<input v-model="form.indexType" /></label>
        <label>预计周期<input v-model="form.cycle" /></label>
        <label>期刊简介<textarea v-model="form.description"></textarea></label>
        <label>上传期刊文档<input type="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt" :disabled="uploading" @change="uploadDocument" /><small>{{ uploading ? "正在上传…" : "支持 PDF、Word、Excel、PPT，最大 20MB" }}</small></label>
      ...
      <div class="list">
        ...
        <article v-for="item in filteredItems" v-else :key="item.id" class="card row journal-row">
          <div><span class="tag">{{ item.type }}</span><b>{{ item.title }}</b><small>{{ item.field }} · {{ item.indexType }} · {{ item.published ? "官网已上架" : "官网未上架" }}<template v-if="item.documentName"> · 已上传文档</template></small></div>
