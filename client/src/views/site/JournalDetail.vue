...
<template>
  ...
  <template v-else-if="journal">
    <section class="detail-hero"><div class="shell detail-hero-grid"><div><span class="tag">{{ journal.type }} · {{ journal.field }}</span><h1>{{ journal.title }}</h1><p>{{ journal.description }}</p><div class="detail-actions"><a class="primary" :href="consultHref(journal.title, journal.type)" :data-consult-subject="journal.title" :data-consult-target="journal.type" onclick="window.__iesstConsultationFromElement && window.__iesstConsultationFromElement(this)">申请适配评估</a><a v-if="journal.documentUrl" class="ghost" :href="journal.documentUrl" target="_blank" rel="noreferrer">下载期刊文档</a><RouterLink class="ghost" :to="`/${journal.type}`">返回列表</RouterLink></div></div><aside class="card metric-panel"><div><span>检索类型</span><b>{{ journal.indexType }}</b></div><div><span>预计周期</span><b>{{ journal.cycle }}</b></div><div><span>服务方式</span><b>一对一顾问</b></div></aside></div></section>
